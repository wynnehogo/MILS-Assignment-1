...
class MiniImageNetDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.channels == 'RGB':
            pass  # use all 3 channels
        elif self.channels in ['RG', 'GB','RB']:
            channel_map = {"R": 0, "G": 1, "B": 2}
            selected = [channel_map[c] for c in self.channels]
            image_np = image_np[:, :, selected]
        else:
            raise ValueError(f"Unsupported channel combination: {self.channels}")
        ...
        # Make sure the image tensor shape is [C, H, W]
        if image.ndim == 2:
...
def train(model, train_loader, val_loader, optimizer, scheduler, num_epochs=3, print_every=100):
...
if __name__ == "__main__":
    ...
    for ch in channels_list:
        ...
        train(model, train_loader, val_loader, optimizer, scheduler, num_epochs=3)
        acc, prec, rec, f1 = evaluate(model, val_loader)
